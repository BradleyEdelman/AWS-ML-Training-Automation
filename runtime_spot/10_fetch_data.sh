...
S3_BUCKET_DATA=$(yq e '.aws.s3_bucket_data' $CONFIG_FILE)
S3_BUCKET_CHECKPOINT=$(yq e '.aws.s3_bucket_checkpoint' $CONFIG_FILE)
...
echo "Downloading dataset from S3 bucket: $S3_BUCKET_DATA..."
aws s3 sync "s3://$S3_BUCKET_DATA/" "$DATASET_PATH/" --exclude "checkpoint*"
...
if [[ "$RESUME_FROM_CHECKPOINT" == "true" ]]; then
	...
	LATEST_CHECKPOINT=$(aws s3 ls "s3://$S3_BUCKET_CHECKPOINT/" | grep "$CHECKPOINT_PREFIX" | sort -k1,2 | tail -n 1 | awk '{print $4}')

	if [[ -n "$LATEST_CHECKPOINT" ]]; then
		echo "Found latest checkpoint: $LATEST_CHECKPOINT. Downloading..."
		aws s3 cp "s3://$S3_BUCKET_CHECKPOINT/$LATEST_CHECKPOINT" "$CHECKPOINT_DIR/$LATEST_CHECKPOINT"
